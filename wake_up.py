...
# CONFIG
# =========================================================================
ROBOT_IP = "192.168.58.4"   # <-- change to your robot IP
dt = 0.008                  # 125 Hz

# =========================================================================
# WAYPOINTS: Toggle per pose
# Comment the CARTESIAN line and uncomment the JOINT line to use joint space
# =========================================================================

# 1. DOCKED
DOCKED = [28.592, -155.366, 292.962, 179.526, 35.861, 45.481]          # Cartesian
# DOCKED = [42.482, -177.699, 156.121, -104.226, -87.770, 87.5682]    # Joint

# 2. WAKEUP
WAKEUP = [52.818, -133.182, 413.154, -135.222, 86.680, 89.540]        # Cartesian
# WAKEUP = [42.482, -176.239, 156.181, -157.584, -87.770, 87.5682]    # Joint

# 3. ARC_MID (reduced bulge)
ARC_MID = [-28.547, -207.695, 473.703, -173.062, 75.089, 51.966]      # Cartesian
# ARC_MID = [42.4825, -146.895, 144.4995, -162.794, -87.7695, 87.5682] # Joint

# 4. ALERT
ALERT = [-26.072, -205.431, 678.455, -117.920, 87.304, 106.820]         # Cartesian
# ALERT = [42.483, -137.551, 116.818, -158.004, -87.769, 87.5682]     # Joint

WAYPOINTS = [DOCKED, WAKEUP, ARC_MID, ALERT]

# Seed for IK (original DOCKED joint pose — keeps solutions close to intended posture)
SEED = [42.482, -177.699, 156.121, -104.226, -87.770, 87.5682]

# =========================================================================
# CUSTOM FK (for IK verification)
# =========================================================================
...
# =========================================================================
# RUCKIG SEGMENT GENERATOR
# =========================================================================
def generate_segment(start_j, end_j, v_max, a_max, j_max,
                     start_vel=None, end_vel=None):
    otg = Ruckig(6, dt)
    inp = InputParameter(6)
    out = OutputParameter(6)
    inp.current_position  = np.deg2rad(start_j).tolist()
    inp.target_position   = np.deg2rad(end_j).tolist()
    inp.current_velocity  = [0.0]*6 if start_vel is None else start_vel
    inp.current_acceleration = [0.0]*6
    inp.target_velocity   = [0.0]*6 if end_vel is None else end_vel
    ...
    while res == Result.Working:
        ...
        traj.append(list(out.new_position))
...
def main():
    ...
    # Comment out this entire block if you uncommented the JOINT waypoints above
    print("[MODE] Cartesian → IK")
    joint_waypoints = solve_ik_sequence(WAYPOINTS, SEED)

    # Option B: JOINT-SPACE direct (bypass IK)
    # Uncomment the line below if you switched the WAYPOINTS to joint angles above
    # joint_waypoints = [np.array(wp) for wp in WAYPOINTS]

    # -----------------------------------------------------------------
    # RUCKIG TRAJECTORY GENERATION
    # -----------------------------------------------------------------
    # Segment 1: DOCKED → WAKEUP (aggressive snap, zero at both ends)
    seg1 = generate_segment(joint_waypoints[0], joint_waypoints[1],
                            v_max=120, a_max=500, j_max=3000)

    # Segment 2: WAKEUP → ARC_MID (flow through, don't stop at ARC_MID)
    blend = np.deg2rad(15)
    seg2 = generate_segment(joint_waypoints[1], joint_waypoints[2],
                            v_max=100, a_max=120, j_max=400,
                            start_vel=[0.0]*6, end_vel=[blend]*6)

    # Segment 3: ARC_MID → ALERT (come to rest)
    seg3 = generate_segment(joint_waypoints[2], joint_waypoints[3],
                            v_max=100, a_max=120, j_max=400,
                            start_vel=[blend]*6, end_vel=[0.0]*6)

    full_traj = np.vstack([seg1, seg2, seg3])
    print(f"Trajectory ready: {len(full_traj)} points ({len(full_traj)*dt:.3f}s)")

    # -----------------------------------------------------------------
    # EXECUTE
    # -----------------------------------------------------------------
    print("\n[1/3] Move to DOCKED (MoveJ)...")
    robot.MoveJ(joint_waypoints[0].tolist(), tool=0, user=0, vel=30)
    time.sleep(0.3)

    print("[2/3] WakeUp + GetUp (ServoJ)...")
    execute_servo(robot, full_traj)

    print("[3/3] Done. Holding ALERT.")
    time.sleep(0.5)
