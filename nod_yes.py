...
# 0. ALERT (start / end pose)
ALERT = [-26.072, -205.431, 678.455, -117.920, 87.304, 106.820]      # Cartesian
# ALERT = [42.483, -137.551, 116.818, -158.004, -87.769, 87.5682]    # Joint

# 1. NOD_BASE
NOD_BASE = [38.465, -147.328, 394.432, -178.728, 53.426, 46.702]      # Cartesian
# NOD_BASE = [38.5398, -176.2388, 147.0908, -114.107, -84.7198, 85.1262]  # Joint

# 2. NOD_DOWN_FWD (J4 down + J2 fwd — combined extreme from original)
NOD_DOWN_FWD = [61.799, -128.742, 470.466, -125.289, 84.401, 98.668] # Cartesian
# NOD_DOWN_FWD = [38.5398, -174.739, 147.0908, -149.107, -84.7198, 85.1262]  # Joint
...
def main():
    ...
    # RUCKIG TRAJECTORY
    # -----------------------------------------------------------------
    traj = generate_joint_trajectory(joint_waypoints, v_max=80, a_max=200, j_max=800)
    print(f"Nod trajectory: {len(traj)} points ({len(traj)*dt:.3f}s)")
